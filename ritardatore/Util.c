...
{
	unsigned int seed;
	if(seme<0)  /* genera seme in base al tempo */
		seed=(unsigned int)getpid();
	else
		seed=(unsigned int) seme;
	srandom( seed );
...
long int msec_elapsed_time(struct timeval prima)
{
	long int msec;
	struct timeval diff, now;

	gettimeofday(&now,NULL);
	diff=differenza(now,prima);
...
int minoreouguale(struct timeval *a, struct timeval *b)
{
	...
	/* printf("uno.sec=%li due.sec=%li    uno.usec=%li due.usec=%li\n", a->tv_sec, b->tv_sec, a->tv_usec, b->tv_usec ); */
